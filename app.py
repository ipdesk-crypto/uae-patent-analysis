...
def refine_data(df):
    ...
    if 'Classification' in df.columns:
        ...
        # Extract the First Letter (A-H) for the new Histogram
...
if df is not None:
    ...
    # --- MODULE 2: CLASSIFICATION & COUNTRY STRENGTH (UPDATED) ---
    elif menu == "Classification & Country Strength":
        st.header("🌍 IPC Strength & Country Activity")
        
        # NEW SECTION: IPC Section Histogram (A-H, excluding T)
        st.subheader("📊 Classification Distribution (Main Sections)")
        # Filter to only valid single letters, excluding "T" or "Ther"
        ipc_sections = filtered_df[
            (filtered_df['IPC_Section'].str.isalpha()) & 
            (filtered_df['IPC_Section'].str.len() == 1) & 
            (filtered_df['IPC_Section'] != 'T')
        ]
        section_counts = ipc_sections.groupby('IPC_Section').size().reset_index(name='Count').sort_values('IPC_Section')
        
        fig_ipc_hist = px.bar(section_counts, x='IPC_Section', y='Count', text='Count', 
                             labels={'IPC_Section': 'IPC Section (A-H)', 'Count': 'Number of Patents'},
        ...
        # Existing Country Activity by IPC logic
